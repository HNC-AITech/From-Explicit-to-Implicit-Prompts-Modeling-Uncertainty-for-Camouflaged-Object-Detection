...
class FTM(nn.Module):
    def __init__(self, channel):
        ...
        self.DeformConv1 = DeformConv_input1(channel, groups=channel)
        self.DeformConv2 = DeformConv_input1(channel, groups=channel)
        self.DeformConv3 = DeformConv_input2(channel, groups=channel)
        self.proj_2 = nn.Conv2d(channel, channel, 1)

    def forward(self, fea):
        fea_en = self.activation(self.proj_1(fea))

        fea_offset = self.DeformConv2(self.DeformConv1(fea))
        fea_en = self.DeformConv3(fea_en, fea_offset)

        out = self.proj_2(fea_en)+ fea
