...
def train(train_loader, model, optimizer, epoch, save_path, ):
    ...
    try:
        for i, (images, gts) in enumerate(train_loader, start=1):
            ...
            size_rates = [1, 1.5]
            # layers = [model.backbone1, model.decoder, model.pre]  # 想要冻结的参数
            # for layer in layers:
            #     for param in layer.parameters():
            #         param.requires_grad = True
            # model.backbone1.train()
            # model.decoder.train()
            # model.pre.train()
            for rate in size_rates:
                trainsize = int(round(opt.trainsize * rate / 32) * 32)
                images_size = F.interpolate(images, size=(trainsize, trainsize), mode='bilinear',align_corners=True)
                gts_size = F.interpolate(gts, size=(trainsize, trainsize), mode='bilinear', align_corners=True)

                optimizer.zero_grad()
                seg = model([images_size], mode=0)

                bce_iou_deep_supervision = CE(seg, gts_size) + iou_loss(seg, gts_size)
            ...
            optimizer.zero_grad()
            seg = model([images, joint_mask], mode=1)

            seg_supervision = CE(seg, gts) + iou_loss(seg, gts)
            reg_supervision = regression_loss(seg.sigmoid(), gts)
            ...
            # reg_supervision = regression_loss(seg.sigmoid(), gts, weight=joint_mask_f) + \
            #                   regression_loss(seg.sigmoid(), gts, weight=joint_mask_b) + \
            #                   regression_loss(seg.sigmoid(), gts, weight=joint_mask_u)
            ...
            if i % 100 == 0 or i == total_step or i == 1:
                print(
                    '{} Stage :{:02d}, Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f}||seg_loss:{:4f}, reg_loss:{:4f}'.
                ...
                logging.info(
                    '#TRAIN#:Epoch [{:03d}/{:03d}], Step [{:04d}/{:04d}], LR:{:.7f},  seg_loss:{:4f},  reg_loss:{:4f}, mem_use:{:.0f}MB'.
                        format(epoch, opt.epoch, i, total_step, optimizer.state_dict()['param_groups'][0]['lr'],seg_supervision.data, reg_supervision.data, memory_used))
...
def test_step(test_loader, model, epoch, save_path):
    ...
    with torch.no_grad():
        ...
        for i in range(test_loader.size):
            ...
            seg = model([image, joint_mask], mode=1)
            res = seg
